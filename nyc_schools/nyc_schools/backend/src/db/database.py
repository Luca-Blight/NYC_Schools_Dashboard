...
import nyc_schools.backend.src.models 
import psycopg2
from nyc_schools.backend import settings
conn_dev = psycopg2.connect(database = "project_one", user = "postgres", password = "postgres", port=5432)

def get_db():
    if "db" not in g:
        g.db = psycopg2.connect(user = current_app.config['DB_USER'],password = current_app.config['DB_PASSWORD'],
            dbname = current_app.config['DATABASE'],port=5432)
    return g.db
...
def drop_all_tables(conn, cursor):
    table_names = ['schools', 'attendance', 'student_population', 'scores']
